...
renderer.setSize(Math.round(window.innerWidth * CANVAS_SCALE), Math.round(window.innerHeight * CANVAS_SCALE), false);
...
function createGroundWireGrid(boundingBox) {
  ...
  // Tile spacing — increase multiplier for larger tiles.
  const xStep = Math.max(size.x * 2, 1e-6);
  const zStep = Math.max(size.z * 2, 1e-6);
...
let unbrokenScaleAnimEnabled = true;
let lowOpacityEnabled = false;
const LOW_OPACITY_CAP = 0.5;

function restartDimensionFrameTimeline() {
  if (!dimensionFrameAnimationEnabled) {
    // Ensure frames stay at their final positions when animation is off
    const ids = ['dimension-layer-4d', 'dimension-layer-3d', 'dimension-layer-2d', 'dimension-layer-1d'];
    const targets = [6, 12, 19, 24];
    ids.forEach((id, i) => {
      const el = document.getElementById(id);
      if (el) el.style.inset = `${targets[i]}%`;
    });
    return;
  }
  if (!dimensionFrameTimeline) return;
...
function resetLoopSceneState(bridgeOpacity) {
  ...
  setFourDMeshOpacity(lowOpacityEnabled ? Math.min(bridgeOpacity, LOW_OPACITY_CAP) : bridgeOpacity);
...
function animateCameraToCube() {
  ...
  cameraLoopTimeline = gsap.timeline({
    ...
    .to(cubes?.material ?? {}, {
      opacity: 0.05, duration: cubesFadeInDuration, ease: 'power2.out', overwrite: false,
      onUpdate() { if (lowOpacityEnabled && cubes?.material) cubes.material.opacity = Math.min(cubes.material.opacity, LOW_OPACITY_CAP); },
    }, revealStart)
    .to(extraCubesFade, {
      opacity: 0.4, duration: cubesFadeInDuration, ease: 'power2.out', overwrite: false,
      onUpdate: () => { if (lowOpacityEnabled) extraCubesFade.opacity = Math.min(extraCubesFade.opacity, LOW_OPACITY_CAP); setExtraCubesOpacity(extraCubesFade.opacity); },
    }, revealStart)
    .to(cubes?.material ?? {}, {
      opacity: 0,
    ...
    .to(bridgeFade, {
      ...
      duration: returnDuration * 0.8,
      ease: 'sine.inOut',
      onUpdate: () => setFourDMeshOpacity(lowOpacityEnabled ? Math.min(bridgeFade.opacity, LOW_OPACITY_CAP) : bridgeFade.opacity),
      onStart: () => {
...
// Toggle low opacity cap for point cloud cubes + mesh
window.toggleLowOpacity = function() {
  lowOpacityEnabled = !lowOpacityEnabled;
  // Apply immediately to current scene state
  if (lowOpacityEnabled) {
    if (cubes?.material) cubes.material.opacity = Math.min(cubes.material.opacity, LOW_OPACITY_CAP);
    setExtraCubesOpacity(Math.min(extraCubesFade.opacity, LOW_OPACITY_CAP));
    if (FourDMesh) setFourDMeshOpacity(Math.min(FourDState.opacity, LOW_OPACITY_CAP));
  }
  return lowOpacityEnabled;
};

// Toggle the #text-unbroken scale/opacity animation on/off
window.toggleUnbrokenScaleAnim = function() {
  unbrokenScaleAnimEnabled = !unbrokenScaleAnimEnabled;
  if (!unbrokenScaleAnimEnabled) {
    gsap.set('#text-unbroken', { scale: 0.2, opacity: 1 });
  }
  return unbrokenScaleAnimEnabled;
};
